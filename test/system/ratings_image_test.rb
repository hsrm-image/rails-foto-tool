...
class RatingsImageTest < ApplicationSystemTestCase
  setup do
  ...
  test "creating a rating on an Image" do
    Capybara.session_name = "first_session"
    visit images_url
    page.all('.grid-element').last.click

    assert_difference "Rating.count", 1 do
      assert_difference "Rating.where(rateable_type: 'Image').count", 1 do
  ...
  test "updating a rating on an Image" do
