class UsersController < ApplicationController
  ...
  def admin
    ...
    respond_to do |format|
      if valid and @user.update({admin: admin})
        format.html { redirect_to @user, notice: t('controllers.updated', resource: t("users.resource_name")) }
  ...
  def create
    ...
    respond_to do |format|
      if @user.save
        format.html { redirect_to @user, notice: t('controllers.created', resource: t("users.resource_name")) }
  ...
  def destroy
    respond_to do |format|
      unless(@user.is_last_admin)
        @user.destroy
        format.html { redirect_to users_url, notice: t('controllers.destroyed', resource: t("users.resource_name")) }
        format.json { head :no_content }
      else
        format.js {render 'layouts/toast', locals: { :method => "error", :message => t("users.model.delete_last"), :title => ""}}
      end
    end
  end
